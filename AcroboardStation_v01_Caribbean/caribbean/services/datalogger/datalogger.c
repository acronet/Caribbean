...
static volatile uint32_t g_acc_task_send_RT = 0;
...
uint8_t datalogger_init(void)
{
	...
	if(g_interv_task_send_RT>60)
		g_acc_task_send_RT = g_interv_task_send_RT - 20;
	else
		g_acc_task_send_RT = 0;
...
static void dl_iterator_moveback_with_cap(AT24CXX_iterator * pIter,uint16_t range)
{
	...
	if(n>k) {
		debug_string(NORMAL,PSTR("(dl_iterator_moveback_with_cap) iterator reached the START position\r\n"),true);
	...
	if(z<x) {
		debug_string_P(NORMAL,PSTR("(dl_iterator_moveback_with_cap) iterator wraps datalogger\r\n"));
...
static void dl_iterator_moveforward_with_cap(AT24CXX_iterator * pIter,uint16_t range)
{
	const int32_t n = range*sizeof(DL_LOG_ITEM);
	const int32_t z = pIter->plain;
	...
	if(n>k) {
		debug_string_P(NORMAL,PSTR("(dl_iterator_moveforward_with_cap) iterator reached the END position\r\n"));
	...
	if(z>x) {
		debug_string_P(NORMAL,PSTR("(dl_iterator_moveforward_with_cap) iterator wraps datalogger\r\n"));
...
static void dl_iterator_moveback(AT24CXX_iterator * pIter,uint16_t range)
{
	...
	if(z<x) {
		debug_string_P(NORMAL,PSTR("(dl_iterator_moveback) iterator wraps datalogger\r\n"));
		pIter->plain = LOG_LAST_VALID_ADDRESS - (x-z);
	} else {
		pIter->plain = z - n;
	}
...
static uint8_t dl_search_record_by_date(uint32_t dt,enum DL_EEPROM_SEARCH_MODE search_mode,AT24CXX_iterator * pIter)
{
	...
	dl_iterator_moveback_with_cap(&it,1);
	...
	while(1){
		...
		if(reg_dir == 0) { //if we are going to search backward
			...
			dl_iterator_moveback_with_cap(&it,reg_skip);
			...
			dl_iterator_moveforward_with_cap(&it,reg_skip);
		}
	}
...
uint16_t send_data_with_post( DL_SEND_PARAMS * const pPara )
{
	...
	uint8_t pg,msb,lsb;
	const uint32_t min_date_boundary = pPara->dt_end;
	...
	while(iter<QUEUE_LEN) {
		...
		if(VERBOSE<=g_sendverb) {
			debug_string_P(g_sendverb,PSTR("\trecord "));
			AT24CXX_iterator_report(&it);
		}
		...
		if(NORMAL<=g_sendverb) {
			char szDebug[64];
			snprintf_P(szDebug,sizeof(szDebug),PSTR("\tRecord timestamp %lu\r\n"),ds.data_timestamp);
			debug_string(NORMAL,szDebug,RAM_STRING);
		}
	...
	//////////////////////////////
		...
		snprintf_P(	szBuf+post_data_end ,
					BUFFER_SIZE-post_data_end ,
					PSTR("#TIME=%lu&B=%u&P=%u&S=%u") ,
					ds.data_timestamp ,
					ds.vBat ,
					ds.raingauge_stats[0].tips,
					mxs0 );
					
		post_data_end += strnlen(szBuf+post_data_end,256);
		
		//debug_string(NORMAL,PSTR("\r\n"),PGM_STRING);
		//debug_string(NORMAL,szBuf+post_data_start,false);
	...
	char szRet[32];
	uint8_t len_szRet=sizeof(szRet);
...
static void dl_snapshot_iter_dec(uint8_t * const idx)
{
	if( *idx == 0 ) {
		(*idx)=(SNAPSHOT_SIZE-1);
	} else {
...
uint8_t  dl_task_store_data()
{
	...
	while(idx!=g_data_snapshot_end) {
		...
		const uint8_t v = it->byte[LSB_BYTE];
		delay_ms(5);//wait before issuing commands on the TWI
